...
// Inst Admin sees: Dashboard, Users, Departments, Teachers, Students, Quizzes, Analytics, Logs
// Does NOT see: Settings (super_admin only)
const NAV = [
...
export default function InstAdminPortal() {
  ...
  // Strict route guard — only inst_admin allowed here
  useEffect(() => {
    if (loading) return;
    ...
    else if (!["inst_admin"].includes(profile.role)) nav("/teacher", { replace: true });
  }, [fbUser, profile, loading]);

  // Quizzes scoped to institute
  useEffect(() => {
    if (!fbUser || !profile?.institute) return;
    setBusyList(true);
    const q = query(
      collection(db, "quizzes"),
      where("institute", "==", profile.institute),
    ...
    <div className="flex h-screen bg-[var(--bg-main)] text-[var(--text-main)] overflow-hidden font-sans antialiased">
      ...
      <aside className="hidden lg:flex w-72 flex-col border-r border-[var(--border-main)] bg-[var(--bg-card)] z-40 shadow-sm">
        <div className="p-8 border-b border-[var(--border-main)] flex items-center gap-3 cursor-pointer group" onClick={() => setActiveTab("home")}>
          <div className="h-10 w-10 flex items-center justify-center rounded-xl bg-violet-600 font-bold text-white shadow-lg group-hover:scale-110 transition-transform">Q</div>
          <div>
            <div className="text-xl font-black tracking-tight text-[var(--text-main)]">QuizVerse</div>
            <div className="text-[10px] font-bold tracking-wider text-violet-500 uppercase">🏛️ Institute Admin</div>
          </div>
        </div>

        {/* Institute badge */}
        {profile?.institute && (
          <div className="mx-4 mt-3 px-4 py-2 bg-violet-500/10 border border-violet-500/20 rounded-2xl">
            <div className="text-[9px] font-bold text-violet-400 uppercase tracking-widest">Institution</div>
            <div className="text-xs font-bold text-[var(--text-main)] truncate mt-0.5">{profile.institute}</div>
          </div>
        )}
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1 mt-2">
          {NAV.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)}
              className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all group ${activeTab === item.id ? "bg-violet-600 text-white shadow-xl shadow-violet-500/20 font-bold" : "text-[var(--text-dim)] hover:bg-[var(--bg-subtle)] hover:text-[var(--text-main)]"}`}>
        ...
        <div className="p-6 border-t border-[var(--border-main)] space-y-4">
          ...
          <div onClick={() => setOpenProfile(true)} className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--bg-subtle)] border border-[var(--border-main)] hover:border-violet-400 cursor-pointer transition-all group">
            <UserAvatar src={profile?.photoURL} name={profile?.name} size="sm" verified />
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-[var(--text-main)] truncate">{profile?.name || "Inst Admin"}</div>
              <div className="text-[9px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Institutional Admin</div>
            </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div className="lg:hidden p-5 border-b border-[var(--border-main)] flex justify-between items-center bg-[var(--bg-card)] sticky top-0 z-50 shadow-sm">
          <div className="flex items-center gap-3" onClick={() => setActiveTab("home")}>
            <div className="h-9 w-9 flex items-center justify-center rounded-xl bg-violet-600 text-white font-bold text-lg shadow-md">Q</div>
            <span className="font-bold text-[var(--text-main)] uppercase tracking-tighter text-lg">Institute Admin</span>
          </div>
        ...
        <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[var(--bg-card)] border border-[var(--border-main)] px-2 flex justify-around items-center h-[76px] w-[92%] max-w-sm rounded-[2.5rem] shadow-2xl">
          {NAV.slice(0, 5).map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center justify-center gap-1.5 w-14 h-14 rounded-2xl transition-all ${activeTab === item.id ? "text-violet-600 font-bold bg-violet-50/50" : "text-[var(--text-dim)]"}`}>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end animate-in fade-in duration-300">
          ...
          <div className="relative w-[85%] max-w-sm bg-[var(--bg-card)] h-full border-l border-[var(--border-main)] shadow-2xl flex flex-col pt-10 px-8 animate-in slide-in-from-right duration-500">
            <div className="flex justify-between items-center mb-8">
              <span className="text-xl font-bold uppercase tracking-tight">🏛️ Inst Admin</span>
            ...
            <div className="space-y-1.5 overflow-y-auto flex-1">
              {NAV.map(item => (
                <button key={item.id} onClick={() => { setActiveTab(item.id); setShowMenu(false); }}
                  className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all ${activeTab === item.id ? "bg-violet-600 text-white font-bold shadow-lg" : "text-[var(--text-dim)] hover:bg-[var(--bg-subtle)]"}`}>
