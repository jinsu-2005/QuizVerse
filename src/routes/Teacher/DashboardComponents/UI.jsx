...
export function UserAvatar({ src, name, size = "md", verified = false }) {
  ...
  return (
    <div className="relative inline-block">
      ...
      {verified && (
        <div className="absolute bottom-0 right-0 bg-emerald-600 text-white rounded-full p-1 border-2 border-[var(--bg-card)] shadow-sm">
  ...
  return (
    <div className={`p-6 rounded-2xl border ${dynamicBorder[color]} bg-[var(--bg-card)] shadow-sm group hover:shadow-md transition-all`}>
      <div className="flex justify-between items-center mb-4">
        ...
        {trend && (
          <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${trend.startsWith("+") ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-500" : "bg-rose-500/10 text-rose-600 dark:text-rose-500"}`}>
            {trend}
          </span>
        )}
      </div>
...
export function Select({ label, value, onChange, options, renderOption }) {
  const normalized = options.map((option) =>
    typeof option === "string" ? { value: option, text: renderOption ? renderOption(option) : option } : option
  );
...
export function Badge({ children, variant = "indigo" }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-md border text-[10px] font-bold uppercase tracking-wider ${variants[variant] || variants.indigo}`}>
      {children}
    </span>
  );
}
