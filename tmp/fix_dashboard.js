const fs = require('fs');
const content = fs.readFileSync('c:/Users/jinsu/Documents/My Projects/Quiz-Verse/src/routes/Teacher/Dashboard.jsx', 'utf8');
...
function QuizRow({ quiz, onDelete, onDuplicate, onEdit }) {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      setStats({ loading: false, count: 0, avg: 0, best: 0, last: null, error: errorMsg });
    }
  };
...
function findMarker(text) {
    // Robust search for RowSkeleton closing part
    // Matches:
    // RowSkeleton() {
    //   return (
    //     ...
    //   );
    // }
    const pattern = /function RowSkeleton\(\) \{[\s\S]*?return \([\s\S]*?\}\s*;/;
    const match = text.match(pattern);
    if (!match) {
       // try without function keyword if it's an arrow function or something
       const pattern2 = /RowSkeleton\s*=\s*\(\)\s*=>\s*\{[\s\S]*?return \([\s\S]*?\}\s*;/;
       const match2 = text.match(pattern2);
       return match2 ? { index: match2.index + match2[0].length, length: match2[0].length } : null;
    }
    return match ? { index: match.index + match[0].length, length: match[0].length } : null;
}

const markerMatch = findMarker(content);
const splitMarker = 'function KPI({ label, value }) {';
const splitIndex = content.indexOf(splitMarker);

if (markerMatch && splitIndex > markerMatch.index) {
    const newContent = content.substring(0, markerMatch.index) + '\\n\\n' + fixedBlock + '\\n\\n' + content.substring(splitIndex);
    fs.writeFileSync('c:/Users/jinsu/Documents/My Projects/Quiz-Verse/src/routes/Teacher/Dashboard.jsx', newContent);
    console.log('File fixed successfully');
} else {
    // try fallback: look for the start of the corrupted block
    // It seems to start after } around line 1773
    console.error('Markers not found', { markerMatch, splitIndex });
    process.exit(1);
}
